...
let URETIM_BILGISI, PERSONEL_DATA, BANT_SONU_DATA, BANT_ICI_DATA, GOREVLER, HATALAR;
let DONEM;
let BANTLAR = [];
let BANT_NOLARI = [];
...
setInterval(() => {
  if (BP === undefined || BP === null || Object.keys(BP).length == 0 || new Date().getTime() - SAYAC > 10 * DAKIKA) {
    bpAl();
  }
}, 5 * DAKIKA);
...
function bantData(baslangicMi) {
  ...
  BANTLAR = urlSorguParametresi(url, "bantlar").split("-");
  BANT_NOLARI = urlSorguParametresi(url, "bantlar").split("-");
  // Ara Kontrol Slide & Data
  araKontrol(baslangicMi);
...
function gunlukData(donemlikData, baslangicMi) {
  $.ajax({
      url: BEYBOWS_URL + "saatlik_takip/bant_gorevlendirme/" + bugun("-", true),
      type: "GET",
    })
    .done(function(response) {
...
function araKontrol(baslangicMi) {
  var simdi = new Date();
  $.ajax({
      url: BEYBOWS_URL + "ara_kontrol/" + bugun("-", true),
      type: "GET",
    })
    .done(function(res) {
      let response=[];
      for(var r = 0; r < res.length; r++){
        if(GOREVLER[res[r].gorevId]!==undefined){ // Sadece seçili bantlara ait bilgiler alınır
          response.push(res[r]);
        }
      }
      response.sort((a, b) =>
      ...
      for (bant of BANT_NOLARI) {
        ...
        BANT_ICI_DATA[bant] = getAraKontrolVerileri(bantBilgisi);
      }
...
async function slide() {
  ...
  while (true) {
    ...
    if (URETIM_BILGISI.length > 0) {
      ...
      saatGoster();
      await sleep(10 * SANIYE);
      saatGizle();

      // Eğer veri varsa her on dakikada bir günceller
      if (new Date().getTime() - SAYAC > 10 * DAKIKA) {
        bantData(false);
    ...
    // SADECE BANT SONU DATA VARSA
      ...
      // Eğer veri varsa her on dakikada bir günceller
      if (new Date().getTime() - SAYAC > 6 * SANIYE) {
        bantData(false);
        SAYAC = new Date().getTime();
      }
      await duyuruAyarla("G");
...
function gorevleriAl() {
  ...
  $.ajax({
    ...
    .done(function(response) {
      for (r of response) {
        // Görevlerin bant numaraları ile eşleşme tablosu oluşturulur
        if(BANT_NOLARI.includes(r.bantNo)){
...
async function duyuruAyarla(bantNo) {
  ...
  for (param of DUYURULAR[bantNo]) {
    ...
    await sleep(param.duration * SANIYE);
  }
  $("#duyuru").html("");
  $('#bantAd').css('z-index', '999');
...
function bantSonundanDOMa(bant, datum = false) {
  ...
  if (BANT_SONU_DATA[bant].topHata != 0 &&
    ...
    var wft =
      ((BANT_SONU_DATA[bant].topHata + BANT_ICI_DATA[bant]) * 100) /
...
function duyurulariAl() {
  fetch('/sinapsTV/announcements.php?date=' + bugun("-", true))
    .then(res => res.json())
    .then((out) => {
      if (out == "")
        DUYURULAR = {};
...
function bpAl() {
  ...
  fetch('/sinapsTV/bpcrud.php?date=' + week)
    .then(res => res.json())
    .then((out) => {
      if (out == "")
        BP = {};
